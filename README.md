...
运行方式
准备数据集
程序首次运行时会自动下载 CIFAR-10 数据集。若下载缓慢，可手动下载 cifar-10-python.tar.gz 并放入 code/data/ 目录（无需解压）。
...
打开 main.py，找到模型定义部分（约第 70-80 行），修改如下：
...
测试外部图片
将待测试的图片（.jpg/.png/.jpeg）放入 code/test_images/ 文件夹，然后运行对应脚本：

bash
python test_simpledla.py   # 使用 SimpleDLA 模型
python test_resnet18.py    # 使用 ResNet18 模型
实验结果
模型	CIFAR-10 测试集准确率	外部图片测试准确率
SimpleDLA	77.85%	约 89%
ResNet18	95.48%	约 99%
常见问题
